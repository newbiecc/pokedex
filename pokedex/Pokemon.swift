            ...
            if let dict = result.value as? Dictionary<String, AnyObject> {
                ...
                if let descArr = dict["descriptions"] as? [Dictionary<String, String>] where descArr.count > 0 {
                    if let url = descArr[0]["resource_uri"] {
                        ...
                        Alamofire.request(.GET, nsurl).responseJSON {         // Network request, dont rely on linear sequences when request,data might come back at different time period 
                            response in let result = response.result
                            ...
                            completed() // whether it sucess or not, complete it so the closure can work 
                        }
                    } else {
                        self._description = ""
                    }
                    ...
                    if let evolutions = dict["evolutions"] as? [Dictionary<String,AnyObject>] where evolutions.count > 0 {
                        ...
                        if let to = evolutions[0]["to"] as? String {    // to = next evo pokemon name
                            
                            if to.rangeOfString("mega") == nil {            // If there is no substring "mega" in the string, can't support mega pokemon
                                if let uri = evolutions[0]["resource_uri"] as? String {
                                    let newStr = uri.stringByReplacingOccurrencesOfString("/api/v1/pokemon", withString: "") //newStr = pokeId/
                                    
                                    let num = newStr.stringByReplacingOccurrencesOfString("/", withString: "")  // num = PokeId
